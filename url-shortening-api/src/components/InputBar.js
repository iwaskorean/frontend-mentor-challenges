...
const InputBar = () => {
  ...
  const fetch = () => {
    axios
      .get(`https://api.shrtco.de/v2/shorten?url=${clientUrl}`, {
        responseType: 'json',
      })
      .then((res) => {
      ...
      .catch((err) => {
        console.log(err);
        alert(
          'It must be a invalid link or an error with the API, Try again later'
        );
        setLoading(false);
      });
  };
  ...
  return (
    <React.Fragment>
      <form className="section__form" onSubmit={onFormSubmit}>
        ...
        />
        <button className="button-cyan">Shorten it!</button>
